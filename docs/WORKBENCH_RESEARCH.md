...
当前基本面方向使用可检查规则：在请求范围与关键数据充分的前提下，收入增长为正且营业利润率为正时给出继续研究的积极信号；收入收缩或营业亏损触发审慎信号。这个规则没有经过预测能力校准，其信心标签也不是概率。可选 Tavily 接口只补充带日期、待人工核验的来源发现，搜索摘要不进入已验证财务事实链。
...
1. 冻结独立的跨公司、跨财年测试集，覆盖重述、标签变化、缺失支出、相同财年不同期间等情形；同时报告正确率、覆盖率、错误回答和拒答。
