...
> 门禁修订记录：第一次接口门禁沿用旧轮次的 1,200 输出 token。时间 Prompt 的研究阶段
> 因 `stop_reason=max_tokens` 没有生成文本或原生引用，严格校验随即停止。该尝试未形成完整
> 五策略数据，也未用于效果判断。按照下述预定的“接口错误可修复”规则，所有策略的共同
> 输出上限在正式门禁前提高到 2,400，并新增 `max_tokens` 立即失败检查；其余方法不变。

> 门禁修订记录 2：完整五策略门禁的 trace 校验全部通过，但 ATLAS 将要求区间
> `2010-01/2025-04` 内的证据子期间 `2020-04` 误判为字符串不等。该问题属于期间比较
> 实现错误，不以标准答案为判断依据。正式实验前将方法版本更新为 `live-atlas-1.1`：
> 对可解析的区间允许其内部单点/子区间，区间外仍为硬冲突；无法解析仍记为 unknown。
> 新增区间内接受与区间外拒绝两个测试后，重新执行完整门禁。
...
2,400 最大输出 token、两阶段结构化方式、题目顺序平衡规则和评价容差都保持一致。
...
- 明确晚于截止日：硬冲突，拒绝该证据；
- 明确目标期间、版本或单位不符：硬冲突，拒绝该证据；要求区间内的证据子期间兼容；
