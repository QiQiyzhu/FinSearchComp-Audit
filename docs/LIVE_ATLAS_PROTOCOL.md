...
> 运行修订记录 3：`live-atlas-1.1` 的 2,400-token 五策略门禁完整通过，但 20×5 运行
> 在第二题时间 Prompt 的研究阶段连续两次稳定触发 `max_tokens`。该未完成目录作废且不进入
> 指标。方法规则保持冻结；为了让五种策略都能完成相同的搜索研究，统一输出上限提高到
> 4,800，并以新协议哈希、新门禁和新完整运行目录重新开始。旧四策略也获得相同上限，
> 所以 ATLAS 不享有额外推理预算。
...
4,800 最大输出 token、两阶段结构化方式、题目顺序平衡规则和评价容差都保持一致。
