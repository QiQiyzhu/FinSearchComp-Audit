...
RESULT_DIR = (
    ...
    / "temporal_clash"
    / "results"
    / "atlas_pit_xbrl_20q_sonnet5_20260813"
)
RESULT_FILES = (
    "README.md",
    "metrics.json",
...
def load_xbrl_result(result_dir: Path = RESULT_DIR) -> dict[str, Any]:
    ...
    if missing:
        raise FileNotFoundError(f"ATLAS-PIT-XBRL result is incomplete: {missing}")
    ...
    if len(outcomes) != 20 or result["manifest"]["valid_runs"] != 40:
        raise ValueError(
            "Published ATLAS-PIT-XBRL result must be the complete 20x2 study"
        )
    return result
...
grid-template-columns:repeat(4,1fr);gap:12px;margin-top:34px}.error-card{background:#111f31;border:1px solid var(--line);
...
def _failure_reason(item: dict[str, str]) -> str:
    if item["plain_action"] == "abstain":
        return "开放网页证据不足，普通Agent拒答"
    if (item.get("plain_answer") or "").startswith("-"):
        return "把题目要求的下降幅度输出成负数"
    return "取数、口径或多步计算产生偏差"
...
def build_homepage(result: dict[str, Any]) -> str:
    ...
    errors = "".join(
        f"""<article class="error-card"><b>{html.escape(row['plain_answer'] or '拒答')} → {html.escape(row['gold_answer'])}</b>
        <h3>{html.escape(row['question_zh'])}</h3><p>{html.escape(_failure_reason(row))}</p></article>"""
        for row in losing_rows
    )
    return f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<meta name="description" content="ATLAS-PIT-XBRL：真实Claude、历史截止日与SEC官方事实。20题准确率100%，普通搜索65%，最终证据泄漏0对12.7%。">
...
ATLAS-PIT-XBRL同时提高准确率、覆盖率、时间元数据完整性，并把候选与最终证据泄漏降为0。</p>
<div class="actions"><a class="button" href="xbrl-study.html">查看20题逐题结果 →</a>
<a class="button secondary" href="atlas-pit-xbrl/trace.jsonl">打开40条有效Trace</a></div></div>
...
<div class="score-line"><span>ATLAS-PIT-XBRL</span><b>{_pct(xbrl['decision_accuracy'])}</b></div>
<div class="meter winner"><i style="width:{xbrl['decision_accuracy']:.0%}"></i></div>
<div class="delta"><span>配对准确率提升<br><small>95% CI: +15～+55pp</small></span><b>+35pp</b></div></aside></div></header>

<main><section class="light" id="result"><div class="wrap"><span class="section-kicker">Accuracy × Point-in-time reliability</span>
<h2 class="section-title">“日期未知”不再冒充“零泄漏”。</h2>
<p class="section-copy">独立PIT审计同时检查392个Web候选来源、原生引用和最终采用证据。联合可靠回答只有在答案正确、
最终证据全部有日期且不晚于截止日、来源字段完整时才计为成功。</p><div class="stats">
<div class="stat"><b>100% vs 65%</b><span>最终数值准确率</span></div><div class="stat"><b>100% vs 90%</b><span>回答覆盖率</span></div>
<div class="stat"><b>0 / 32 vs 80 / 392</b><span>候选未来来源</span></div><div class="stat"><b>0 / 84 vs 7 / 55</b><span>最终未来证据</span></div>
<div class="stat"><b>100% vs 30.6%</b><span>候选时间元数据覆盖</span></div><div class="stat"><b>100% vs 36.4%</b><span>最终证据时间元数据覆盖</span></div>
<div class="stat"><b>100% vs 0%</b><span>联合可靠回答率</span></div><div class="stat"><b>7 / 13 / 0</b><span>数值逐题胜 / 平 / 负</span></div></div>
<div class="boundary"><b>结论边界：</b>100%只属于这20道可映射到SEC XBRL的历史财报数值题；
单模型、单次运行不足以宣称开放域RAG或完整FinSearchComp的通用SOTA。</div></div></section>

<section class="dark" id="method"><div class="wrap"><span class="section-kicker" style="color:var(--blue)">Integrated system</span>
<h2 class="section-title">语义交给Claude，历史事实交给SEC，计算交给程序。</h2>
<div class="flow"><article><small>01</small><h3>语义编译</h3><p>识别公司、指标、财年和公式类型。</p></article>
<article><small>02</small><h3>程序校准</h3><p>Label-free语法固定操作数顺序与方向。</p></article>
<article><small>03</small><h3>Point-in-time取数</h3><p>只保留filed ≤ cutoff的10-K事实。</p></article>
<article><small>04</small><h3>确定性计算</h3><p>Decimal执行白名单公式，最后统一舍入。</p></article>
<article><small>05</small><h3>三层审计</h3><p>候选、引用、最终证据分别检查日期与来源。</p></article></div>
<div class="compare-grid"><article class="method"><h3>普通搜索 Agent</h3><div class="big">65%</div><ul>
<li><strong>50</strong>次真实Web Search，392个候选来源</li><li><strong>19/20</strong>题暴露确认的未来来源</li>
<li><strong>6/20</strong>题最终采用未来证据</li><li>模型同时负责搜索、抄数、公式和舍入</li></ul></article>
<article class="method winner"><h3>ATLAS-PIT-XBRL</h3><div class="big">100%</div><ul>
<li><strong>16</strong>次SEC下载 + 68次缓存命中</li><li><strong>0</strong>个未来或未知日期来源</li>
<li><strong>84</strong>条SEC事实citations，来源字段100%完整</li><li>模型规划，官方历史事实，程序执行</li></ul></article></div></div></section>
...
<h2 class="section-title">普通搜索错或拒答的7题，ATLAS全部修复。</h2><p class="lead">数值错误来自方向、操作数、口径和多步计算；
时间错误来自搜索引擎返回今天仍相关、但在历史截止日之后才出现的页面。</p><div class="error-grid">{errors}</div></div></section>

<section class="light" id="papers"><div class="wrap"><span class="section-kicker">Research grounding</span>
<h2 class="section-title">把最新顶会思想落成可运行、可审计的金融系统。</h2><div class="papers">
<a class="paper" href="https://aclanthology.org/2026.eacl-long.322/"><small>EACL 2026 · Long</small><h3>Query Decomposition for RAG</h3><p>将财务公式拆成2–8个有序事实请求。</p></a>
<a class="paper" href="https://aclanthology.org/2026.findings-acl.187/"><small>ACL 2026 · Findings</small><h3>FinMRAGBench</h3><p>以SEC事实、accession和程序trace支持多步金融分析。</p></a>
<a class="paper" href="https://aclanthology.org/2025.acl-long.1089/"><small>ACL 2025 · Long</small><h3>ChainRAG</h3><p>显式保留公司、指标、年度和操作数顺序。</p></a>
...
<p style="color:var(--muted)">20/20 Gold通过SEC复算；方法与成功判据冻结于<code>7a138a3</code>，
一次HTTP 524替代尝试修订记录于<code>ffda118</code>。</p><code>{html.escape(gold_hash)}</code></div>
<div><h3>公开产物</h3><div class="chips"><span>40 valid traces</span><span>1 excluded 524</span>
<span>84 SEC citations</span><span>0 invalid record</span><span>Claude Sonnet 5</span></div></div></div></div>
<div class="artifact-links" style="margin-top:18px"><a href="xbrl-study.html">逐题研究报告</a><a href="atlas-pit-xbrl/case_outcomes.csv">结果CSV</a>
<a href="atlas-pit-xbrl/metrics.json">综合指标</a><a href="atlas-pit-xbrl/gold_audit.json">Gold审计</a>
<a href="atlas-pit-xbrl/exclusions.json">排除记录</a><a href="atlas-pit-xbrl/trace.jsonl">完整Trace</a></div></div></section></main>
<footer class="footer"><div class="wrap footer-row"><span>FinSearchComp-Audit · ATLAS-PIT-XBRL · 2026</span>
<span>真实模型 · 历史截止日 · 官方SEC数据 · 不构成投资建议</span></div></footer></body></html>"""
    ...
    for index, item in enumerate(result["outcomes"], start=1):
        ...
        rows.append(
            ...
            "<td><span class=\"tag\">{}</span></td></tr>".format(
                index, html.escape(item["question_zh"]), html.escape(item["gold_answer"]),
                html.escape(item["canonical_unit"]), "ok" if plain_ok else "bad",
                "✓" if plain_ok else "✕", html.escape(item["plain_answer"] or "拒答"),
                "ok" if xbrl_ok else "bad", "✓" if xbrl_ok else "✕",
                html.escape(item["xbrl_answer"] or "拒答"), item["plain_candidate_future_sources"],
                item["xbrl_candidate_future_sources"], item["plain_final_future_evidence"],
                item["xbrl_final_future_evidence"], "ok" if item["xbrl_joint_reliable"] == "1" else "bad",
                item["plain_joint_reliable"], item["xbrl_joint_reliable"], html.escape(item["operation"]),
            )
        )
    return f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="ATLAS-PIT-XBRL 20题答案与时间可靠性逐题结果">
<title>20题综合正式实验 · ATLAS-PIT-XBRL</title><style>{_shared_css()}</style></head><body>
<header class="study-hero"><nav class="nav wrap"><a class="brand" href="index.html">← FinSearchComp<i> / ATLAS-PIT</i></a>
<div class="navlinks"><a href="atlas-pit-xbrl/README.md">研究卡</a><a href="atlas-pit-xbrl/trace.jsonl">Trace</a><a href="atlas-pit-xbrl/gold_audit.json">Gold审计</a></div></nav>
<div class="wrap" style="padding-top:36px"><span class="eyebrow"><i class="dot"></i>20 New Questions · Historical Cutoffs · 40 Valid Traces</span>
<h1 style="max-width:980px">ATLAS-PIT-XBRL<br><span class="gradient">答案正确且不穿越</span></h1>
<p class="lead">同一个Claude Sonnet 5：普通Web Search与“LLM编译 + 截止日SEC XBRL + Decimal + 三层PIT审计”的完整配对。</p>
<div class="facts-grid"><div class="fact"><b>{_pct(plain['decision_accuracy'])}</b><span>普通搜索准确率</span></div>
<div class="fact"><b>{_pct(xbrl['decision_accuracy'])}</b><span>ATLAS准确率</span></div>
<div class="fact"><b>+{comparison['paired_accuracy_difference'] * 100:.0f}pp</b><span>配对提升（95% CI +15～+55）</span></div>
<div class="fact"><b>7 / 13 / 0</b><span>数值胜 / 平 / 负</span></div></div></div></header>
<main class="light"><div class="wrap"><span class="section-kicker">Per-question audit</span>
<h2 class="section-title">20道题，每个答案和每次时间暴露都可检查。</h2><p class="section-copy">“未来候选”和“最终未来证据”列均为普通Agent / ATLAS；
联合可靠要求答案正确、最终证据全部有日期且不晚于截止日、来源字段完整。</p>
<div class="table-shell"><table><thead><tr><th>#</th><th>问题</th><th>Gold</th><th>普通搜索</th><th>ATLAS</th>
<th>未来候选 P/A</th><th>最终未来 P/A</th><th>联合可靠 P/A</th><th>程序</th></tr></thead><tbody>{''.join(rows)}</tbody></table></div>
<div class="stats"><div class="stat"><b>50</b><span>普通Agent Web Searches</span></div><div class="stat"><b>392</b><span>普通Agent候选来源</span></div>
<div class="stat"><b>80 → 0</b><span>确认未来候选来源</span></div><div class="stat"><b>7 → 0</b><span>最终采用未来证据</span></div></div>
<div class="boundary"><b>统计边界：</b>数值准确率配对bootstrap 95% CI为+15至+55个百分点；
20题、单模型、单次运行仍不足以宣称通用SOTA。1次HTTP 524已公开排除并仅替代一次。</div>
<h2 class="section-title" style="margin-top:62px">下载审计产物</h2><div class="artifact-links"><a href="atlas-pit-xbrl/case_outcomes.csv">逐题CSV</a>
<a href="atlas-pit-xbrl/metrics.json">综合指标</a><a href="atlas-pit-xbrl/gold_audit.json">Gold审计</a><a href="atlas-pit-xbrl/study_manifest.json">协议清单</a>
<a href="atlas-pit-xbrl/exclusions.json">排除记录</a><a href="atlas-pit-xbrl/trace.jsonl">40有效 + 1排除Trace</a></div></div></main>
<footer class="footer"><div class="wrap footer-row"><span>ATLAS-PIT-XBRL formal live study</span><span>Preregistered · Point-in-time · SEC-grounded</span></div></footer>
</body></html>"""
